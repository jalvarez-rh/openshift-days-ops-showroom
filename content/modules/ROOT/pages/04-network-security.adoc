...
[source,bash,role="execute"]
----
oc exec -n netpol-backend deployment/backend -- bash -c "timeout 3 bash -c 'echo > /dev/udp/8.8.8.8/53' 2>&1 && echo 'DNS to 8.8.8.8: ALLOWED' || echo 'DNS to 8.8.8.8: BLOCKED (timed out)'"
----
