...
3. The **Overview** tab shows you the node's health at a glance:
   - **Status** - the box at the top shows the overall node status, key issues (like CPU or memory overcommit warnings), and any active alerts. On this workshop cluster you'll see several alerts — this is normal with many operators installed. The overcommit warnings mean the total **limits** of all pods exceed the node's capacity, but look at the Utilization section below to see that actual usage is well within capacity.
