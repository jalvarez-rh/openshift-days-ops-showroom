...
3. The **Overview** tab immediately shows you:
   - **Status** - at the top you'll see the node is **Ready**, followed by resource warnings in plain English — a red circle for CPU overcommit and a yellow circle for memory approaching capacity. Below that you'll see firing alerts like `ClusterNotUpgradeable` and `AlertmanagerReceiversNotConfigured`. The overcommit warnings mean the **total limits** of all pods exceed the node's capacity — if every pod tried to burst to its maximum simultaneously, the node couldn't handle it and would throttle pods.
+
