...
3. The **Overview** tab shows you the node's health at a glance:
   ...
   - **Utilization** - CPU, Memory, Filesystem, and Network over time. Compare "available" to actual usage — that tells you the real capacity story, not the theoretical limits. The difference between requests (what's guaranteed) and limits (what pods are allowed to burst to) is important. In practice, all pods bursting at once rarely happens, especially in a workshop environment. In production, you'd either right-size your limits, add nodes, or accept the overcommit if your workloads don't burst together.
